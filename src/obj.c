             ...
             )
{
	...
	float normal_array[3*MAX_VERTICES];
	float vertex_array[3*MAX_VERTICES];
	...
	translation_mat = translate(mat4(), vec3(position.x, position.y, position.z));
	rotation_mat = 
		rotate(mat4(), rotation.z, vec3(1.0f, 0.0f, 0.0f)) *
		rotate(mat4(), rotation.y, vec3(1.0f, 0.0f, 0.0f)) *
		rotate(mat4(), rotation.x, vec3(1.0f, 0.0f, 0.0f));
	scale_mat = glm::scale(mat4(), vec3(scale.x,scale.y,scale.z));
	Model =  rotation_mat * translation_mat * scale_mat;
    // Our ModelViewProjection : multiplication of our 3 matrices
	mat4 MVP        = game.Projection * game.View * Model; // Remember, matrix multiplication is the other way around
	...
	for (i = 0; i < obj->num_tris; i++)
    {
		...
		for(j = 0;j < 3;j++)
		{
			if (obj->normal_array)
			{
				normal_array[n++] = obj->normal_array[triangle->p[j].n * 3],
				normal_array[n++] = obj->normal_array[triangle->p[j].n * 3 + 1];
				normal_array[n++] = obj->normal_array[triangle->p[j].n * 3 + 2];
			}
			if (obj->texel_array)
			{
			   texel_array[t++] = obj->texel_array[triangle->p[j].t * 2];
			   texel_array[t++] = obj->texel_array[triangle->p[j].t * 2 + 1];
			}

			vertex_array[v++] = obj->vertex_array[triangle->p[j].v * 3];
			vertex_array[v++] = obj->vertex_array[triangle->p[j].v * 3 + 1];
			vertex_array[v++] = obj->vertex_array[triangle->p[j].v * 3 + 2];
			//Reset barycentric coordinates to 0 for this vertex
			bary_array[b++] = 0.0f;
			bary_array[b++] = 0.0f;
			bary_array[b++] = 0.0f;
			//Set the component matching this triangle vertex to 1.0f for barycentric coordinates
			bary_array[b-3+j] = 1.0f;
		}
    }
	//Setup the data for the vertices
	glEnableVertexAttribArray(0);
	glBindBuffer(GL_ARRAY_BUFFER, game.vertexbuffer);
    glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * v, vertex_array, GL_STATIC_DRAW);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, (void*)0);

	//Setup the data for the UV coordinates and texture
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(GLfloat) * n, normal_array, GL_STATIC_DRAW);
	glVertexAttribPointer(2, 3, GL_FLOAT, GL_FALSE, 0, (void *)0);

	//Setup the data for the barycentric coordinates
	...
	/*if(IS_SET(game.graphics_flags, GFXFLAG_WIREFRAME))
	{
		...
		glUniform3fv(ShaderPrograms[game.shader].uniforms[WIRESHADER_COLOR], 3, 
					 ShaderPrograms[game.shader].values);
		//Set the outline thickness
		glUniform1f(ShaderPrograms[game.shader].uniforms[WIRESHADER_DISTANCE], 
